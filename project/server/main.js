...
Meteor.methods({
    getAnzahlBestellungen2:(customerNumber)=>{
        var results=Amqpdata.find({customerNumber:customerNumber}).count();
        return results;
    },
    getCustomerInfos:()=>{
        var kundendaten=[];
        ...
        for (var z=0;z<kundennummern.length;z++){
            var timestampSize=Amqpdata.find({customerNumber: kundennummern[z]}, {limit: 1, sort: {timeStamp: -1},fields:{timeStamp:1}}).fetch();
